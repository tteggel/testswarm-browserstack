...
var TestSwarmBrowserStackInteg = {
    ...
    getWorkersByBrowser:function (browser, currentWorkers, includeQueued) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (worker.browser === browser.browser && 
                worker.device === browser.device &&
                worker.os === browser.os &&
                worker.version === browser.version &&
    ...
    updateBrowsers:function (currentWorkers, swarmState) {
        ...
        for (browserID in browserMap) {
            ...
            doKillWorkers = false;
            if (stats.pendingRuns > 0 || stats.pendingReRuns > 0) {
                ...
                if (stats.onlineClients === 0 && workers.length && self.options().kill) {
                    ...
                    // 
                    // Disable this for now to give browsers a chance to spin up.
                    // doKillWorkers = true;
                }
            } else if (stats.activeRuns === 0 && stats.pendingReRuns === 0 && stats.pendingRuns === 0 && workers.length && self.options().kill) {
        ...
        var n = Math.min(currentWorkers.length - killWorkers.length, self.options().stackLimit);
